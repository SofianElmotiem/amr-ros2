...
class ROS_env:
    ...
    def step(self, lin_velocity=0.0, ang_velocity=0.1):
        ...
        rclpy.spin_once(self.sensor_subscriber, timeout_sec=0.5)
        while (self.sensor_subscriber.latest_scan is None or
               self.sensor_subscriber.latest_position is None):
    ...
    def reset(self):
        self.cmd_vel_publisher.publish_cmd_vel(linear_velocity=0.0, angular_velocity=0.0)
    ...
    def set_robot_position(self):
        ...
        while not pos:
            ...
            y = np.random.uniform(-4.0, 4.0)
            pos = self.check_position(x, y, 1.8)
        self.set_position("my_bot", x, y, angle)
        return x, y
    ...
    def check_collision(self, laser_scan):
        ...
        if len(finite) == 0:
            return False
        return bool(finite.min() < self.collision_delta)
    ...
    def get_reward(goal, collision, action, laser_scan):
        if goal:
            ...
            scan = np.array(laser_scan)
            finite = scan[np.isfinite(scan)]
            min_scan = float(finite.min()) if len(finite) > 0 else 7.0
            r3 = lambda x: 1.35 - x if x < 1.35 else 0.0
            return action[0] - abs(action[1]) / 2 - r3(min_scan) / 2
